...
def get_db_conn():
    return psycopg2.connect(
        ...
        dbname="capstone_soc",
        user="api_user",
        password="Capstonecybersecurity!"
    )
...
@app.route('/api/add-attack', methods=['POST'])
def add_attack():
    ...
    source = data.get('source', 'victoria')
    ...
    try:
        ...
        cur.execute(
            """
            INSERT INTO victoria_attack_events
            (attack_type, attack_subtype, payload, success, source, source_ip, target, session_id, event_time_utc, extra)
...
def add_detection():
    ...
    source_ip = data.get('source_ip') or data.get('ip_address') or '0.0.0.0'
    description = data.get('description', f"{event_type} detected from {source_ip}")
    ...
    try:
        ...
        cur.execute(
            """
            INSERT INTO detection_events (source, event_type, severity, source_ip, description, extra)
        ...
        cur.close()
        conn.close()
        detection['db_id'] = str(db_id)

    except Exception as e:
        detection['db_error'] = str(e)
    ...
    if detection['severity'] in ['high', 'critical']:
        ...
        block_result = block_malicious_ip(
            detection['ip_address'],
            f"Automated block for {detection['event_type']}"
        )
